<template>
	<div class="course-project">
		<el-dialog title="Tips" :visible.sync="dialogVisible" width="40%">
			<p>
				课程项目由教师发布，限时完成。超时则无法提交，如这里展示的“已截止”的项目。
			</p>
					...
					<div class="detail-section-head">
						...
						<div class="d-right">
							...
							<div class="detail-time">
								{{ formatTime(this.project.startTime) }}
							</div>
							<div class="d-right-title">截止时间：</div>
							<div class="detail-time">
								{{ formatTime(this.project.endTime) }}
							</div>
						</div>
					</div>
			...
			>
				...
				<el-form-item label="项目介绍" prop="desc">
					<el-input
						style="width: 400px"
						type="textarea"
						v-model="createProject.desc"
					></el-input>
				</el-form-item>
				<el-form-item label="实验项目分数" prop="fullMark">
					<el-input
						style="width: 400px"
						v-model.number="createProject.fullMark"
					></el-input>
				</el-form-item>
				...
				<el-form-item>
					<el-button
						type="primary"
						@click="submitProject('createProject')"
						>立即创建</el-button
					>
					<el-button @click="resetForm('createProject')"
						>重置</el-button
					>
				</el-form-item>
			</el-form>
		...
		<div class="main">
			<div class="item" v-for="project in projectList" :key="project.id">
				...
				<div class="item-right">
					...
					<div class="item-finished" v-show="!isOver(project)">
						截止于：{{ endDate(project) }}
					</div>
					<div class="item-finished" v-show="isOver(project)">
						已截止
					...
					<div
						...
						v-show="
							...
							(userType == 'student' && !project.submited)
						"
					...
					<div
						class="item-btn"
						v-show="userType == 'student' && project.submited"
						@click="showDetail(project)"
					>
						已提交
					</div>
...
export default {
	...
	data() {
		...
		return {
			...
			rules: {
				...
				fullMark: [{ validator: checkfullMark, trigger: "blur" }],
				desc: [
					{
						required: false,
						message: "请填写介绍信息",
	...
	mounted() {
		...
		API.course
			.findProject(qs.stringify(this.course))
			.then(res => {
	...
	methods: {
		...
		showDetail(project) {
			...
			this.loadProject(this.isOver(project));
		},
		hideDetail() {
		...
		async createProjectFunc() {
			return new Promise((resolve, reject) => {
				...
				axios
					.createProject(qs.stringify(this.createProject))
					.then(res => {
		...
		async addProjectFile() {
			return new Promise((resolve, reject) => {
				...
				axios
					.addProjectFile(qs.stringify(this.projectFile))
					.then(res => {
						console.log(res);
						this.openPublish = false;
		...
		async addHomework() {
			...
			console.log(this.homework);
			axios
				.addHomework(qs.stringify(this.homework))
				.then(res => {
					console.log(res);
					alert("提交作业成功");
					this.openDetail = false;
