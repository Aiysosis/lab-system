<template>
	<div class="practice">
		<el-dialog title="Tips" :visible.sync="dialogVisible" width="40%">
			<p>
				对抗练习由教师发布，限时完成，超时则无法进入。对抗练习有时间限制，超时会强制提交并退出。练习得分由系统自动给出并计入成绩。
			</p>
				...
				<div class="announce-detail-main">
					<div class="form-area">
						...
						>
							...
							<span
								v-for="(item, index) in this.form.problems"
								:key="index"
							>
								<el-form-item
									style="
								...
								>
									<el-input
										...
										:rows="2"
										v-model="
											form.problems[index].description
										"
									></el-input>
								</el-form-item>
								<el-radio-group
									style="
										...
										margin-bottom: 20px;
									"
									v-model="form.problems[index].type"
								>
								...
								<el-checkbox-group
									...
									"
									class="option-box"
									:max="item.type == 0 ? 1 : 999"
									v-model="form.problems[index].answer"
								>
									<el-checkbox
							...
							<el-form-item>
								<el-button
									type="primary"
									@click="onPracticeCreate()"
									>立即创建</el-button
								>
								<el-button @click="cancleCreate()"
									>取消</el-button
								>
							</el-form-item>
						</el-form>
		...
		<div class="interact-container" v-show="this.startBtn">
			<div class="start-box">
				<div class="start-words">
					本次对抗练习的时间为{{
						this.selectedPractice.timeLimit
					}}分钟
				</div>
				<div class="start-btns">
					<div class="interact-btn" @click="startPractice()">
						开始答题
					</div>
					<div class="interact-btn" @click="exitPractice()">
						退出答题
					</div>
				</div>
			</div>
			...
			<div class="home-work-list-main">
				<div class="problem-area">
					<div class="problem-area-head">
						<div class="min">{{ this.leftMin }}</div>
						分
						<div class="sec">{{ this.leftSec }}</div>
						秒
					</div>
				...
				<div class="answer-area">
					...
					<div class="submit-content">
						<div class="submit-btn" @click="submitPractice()">
							提交
						</div>
					</div>
				</div>
		...
		<div
			class="head-bar"
			v-show="userType == 'teacher' || userType == 'responsible_teacher'"
		>
			<div class="head-bar-left">
				<div class="head-bar-button" @click="openCreate()">
					创建对抗练习
				</div>
			</div>
			<div class="head-bar-right"></div>
		</div>
		<div class="main">
			<div class="item" v-for="item in this.practiceList" :key="item.id">
				...
				<div class="item-right">
					<div
						class="item-finished"
						v-show="!isOver(item) && !item.marked && !item.submited"
					>
						截止于：{{ endDate(item) }}
					</div>
					<div
					...
					<div
						...
						v-show="
							(userType == 'teacher' ||
								userType == 'responsible_teacher') &&
							!item.marked
						"
						@click="caculateGrade(item)"
					>
						统计成绩
					</div>
					<div
...
export default {
	data() {
		return {
			...
			rules: {
				...
				problems: [
					{ required: true, message: "请完成问题", trigger: "blur" },
				],
			},
			createForm: false,
			openList: false,
	...
	methods: {
		onPracticeCreate() {
			...
			API.course.createPractice(qs.stringify(data)).then(res => {
				console.log(res);
		...
		caculateGrade(item) {
			...
			axios.caculatePracticeGrade(qs.stringify(data)).then(res => {
				alert(res.data);
			});
		},
		loadPracticeList() {
			...
			API.course.getPracticeByCourseId(qs.stringify(data)).then(res => {
		...
		loadProblems() {
			return new Promise(resolve => {
				...
				API.course
					.getProblemsByPracticeId(qs.stringify(data))
					.then(res => {
						let list = res.data;
						console.log(list);
						list.forEach(item => {
		...
		submitPractice() {
			...
			API.course.submitPractice(qs.stringify(data)).then(res => {
				this.closePracticeList();
				this.$message({
					message: "提交成功",
					type: "success",
