<template>
	<div class="course-grade">
		<el-dialog title="Tips" :visible.sync="dialogVisible" width="40%">
			<p>
				课程项目成绩分为三个部分：平时成绩、项目成绩、练习成绩。需要课程在创建时由教师指定三者的权重，然后由系统自动计算、实时更新。
			</p>
		...
		<div class="main">
			...
			<div class="item">
				<div class="item-section-one">
					...
					<div class="item-mark">
						{{ project.toFixed(2) }}/{{ projectAll }}
					</div>
				</div>
				<div
					class="item-detail"
					v-for="(item, index) in this.projectGrade"
					:key="index"
				>
					{{ item.project.name }}：{{
						(item.project.fullMark * item.score) / 100
					}}/{{ item.project.fullMark }}
				</div>
			</div>
			<div class="item">
				<div class="item-section-one">
					...
					<div class="item-mark">
						{{ practice.toFixed(2) }}/{{ practiceAll }}
					</div>
				</div>
				<div
					class="item-detail"
					v-for="(item, index) in this.practiceGrade"
					:key="index"
				>
...
export default {
	mounted() {
		...
		API.course.findByCourseId(qs.stringify(data)).then(res => {
			//console.log(res.data);
			this.course = res.data;

			API.course.findScoresByCourse(qs.stringify(data)).then(res => {
				//console.log(res.data);
				let list = [];
				list = res.data;
				list.forEach(item => {
				...
				if (this.usualGrade.score > this.usualAll)
					this.usual = this.usualAll;
	...
	computed: {
		myGradeAll: function () {
			return (
				this.myGrade.usual.grade +
				this.myGrade.project.grade +
				this.myGrade.practice.grade
			);
		},
		usualP: function () {
		...
		projectP: function () {
			let res =
				(this.myGrade.project.grade / this.myGrade.project.all) * 100;
			return res.toFixed(2);
		},
		practiceP: function () {
			let res =
				(this.myGrade.practice.grade / this.myGrade.practice.all) * 100;
			return res.toFixed(2);
		},
		allP: function () {
