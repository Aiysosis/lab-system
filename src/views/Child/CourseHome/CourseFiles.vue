	...
	<div class="course-files">
		<div class="home-work-list-container" v-show="openList">
			<div class="files-box">
				<div class="home-work-list-head">
					<div
						class="list-back-icon"
						@click="closePracticeList()"
					></div>
		...
		<div class="head-bar" v-show="identify()">
			<div class="head-bar-left">
				<div class="head-bar-button" @click="showUpload">
					上传教学资料
				</div>
			</div>
		</div>
		...
		>
			...
			>
				...
				<el-form-item>
					<el-button
						type="primary"
						@click="submitAnnounce('courseFile')"
						>立即创建</el-button
					>
...
export default {
	...
	methods: {
		...
		openPracticeList(title) {
			...
			if (title === "课程学习资料") this.fileType = "course";
			if (title === "实验项目资料") this.fileType = "project";
			if (title === "对抗练习资料") this.fileType = "exercise";

			let data = {
				courseId: this.courseId,
				type: this.fileType,
			};
			API.course.findCourseFiles(qs.stringify(data)).then(res => {
		...
		async addCourseFile() {
			...
			axios
				.addCourseFile(qs.stringify(this.courseFile))
				.then(res => {
					console.log(res);
