<template>
	<div class="home-work">
					...
					<el-table-column label="操作">
						<template slot-scope="scope">
							<div class="operation">
								...
								<div
									class="item-btn"
									v-show="!scope.row.isGrade"
									@click="
										openSetScoreDialog(scope.row.creator.id)
									"
								>
									打分
								</div>
		...
		<el-dialog
			title="作业批改"
			:visible.sync="openSetScore"
			@close="resetForm('setScoreObject')"
		>
			<el-form :model="setScoreObject" ref="setScoreObject">
				...
				>
					<el-input
						type="number"
						v-model.number="setScoreObject.score"
						autocomplete="off"
					></el-input>
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
				<el-button type="primary" @click="setScore('setScoreObject')"
					>确 定</el-button
				>
				<el-button @click="resetForm('setScoreObject')"
					>取 消</el-button
				>
			</div>
		</el-dialog>
		...
		<div class="main">
			<div class="item" v-for="project in projectList" :key="project.id">
				...
				<div class="item-right">
					<div class="item-btn" @click="openHomeworkList(project.id)">
						开始批改
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		setScore(formName) {
			this.$refs[formName].validate(valid => {
				if (valid) {
					...
					console.log(this.setScoreObject);
					axios
						.setProjectScore(qs.stringify(this.setScoreObject))
						.then(res => {
							console.log(res);
							this.openSetScore = false;
		...
		getHomeworkList() {
			API.course
				.getHomeworkList(qs.stringify(this.project))
				.then(res => {
					console.log(res);
