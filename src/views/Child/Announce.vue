<template>
	<div class="account-container">
		<el-dialog title="Tips" :visible.sync="dialogVisible" width="40%">
			<p>
				日历系统中展示了最近的事件，方便用户进行时间规划和管理。同时，事件采用了不同的颜色标注，以区分全局事件、课程事件等。
			</p>
				...
				<div class="clendar-main">
					<el-calendar v-model="value" id="calendar">
						...
						<template slot="dateCell" slot-scope="{ date, data }">
							...
							<div
								:class="{
									'clendar-event': item.type == 0,
									'clendar-event1': item.type == 1,
									'clendar-event2': item.type == 2,
								}"
								v-for="item in getEventByDay(data.day)"
								:key="item.id"
							>
								{{ item.content }}
							</div>
		...
		<div class="announce-detail" v-show="openDetail">
			...
			<div class="appendix" v-show="hasAppendix">
				...
				<div class="content">
					<div
						class="content-main"
						v-for="file in announceDetail.files"
						:key="file.id"
					>
		...
		<el-dialog
			...
			@close="resetForm('publishAnnounce')"
		>
			<el-form
				:model="publishAnnounce"
				:rules="rules"
				ref="publishAnnounce"
				label-width="200px"
				class="demo-ruleForm"
			>
				...
				<el-form-item label="公告内容" prop="content">
					<el-input
						type="textarea"
						v-model="publishAnnounce.content"
					></el-input>
				</el-form-item>
				...
				<el-form-item>
					<el-button
						type="primary"
						@click="submitAnnounce('publishAnnounce')"
						>立即创建</el-button
					>
					<el-button @click="resetForm('publishAnnounce')"
						>重置</el-button
					>
				</el-form-item>
			</el-form>
			...
			<div class="main-container">
				<div class="head-bar" v-show="identify()">
					<div class="head-bar-left">
						<div class="head-bar-button" @click="showPublish">
							发布公告
						</div>
					</div>
				...
				<div class="announce-container">
					<div
						class="item"
						v-for="item in this.announceList"
						:key="item.id"
					>
						<div class="item-left">
						...
						<div class="item-right">
							...
							<div class="item-btn" @click="showDetail(item.id)">
								查看详情
							</div>
						</div>
					</div>
...
export default {
	...
	mounted() {
		...
		this.loadAnnounce();
		API.event.getEvent(qs.stringify({})).then(res => {
			console.log(res);
			let list = res.data;
			list.forEach(item => {
				let t = {
	...
	methods: {
		identify() {
			if (
				this.$store.state.user.type === "responsible_teacher" ||
				this.$store.state.user.type === "teacher"
			)
				return true;
			else return false;
		},
		...
		loadAnnounce() {
			//获取公告
			let data = {};
			API.announce.getAllAnnounce(qs.stringify(data)).then(res => {
		...
		showDetail(id) {
			...
			this.announceDetail.time = moment(
				this.announceList[index].createdTime
			).format("YYYY-MM-DD");
		...
		async createAnnounce() {
			return new Promise((resolve, reject) => {
				axios
					.createNotice(qs.stringify(this.publishAnnounce))
					.then(res => {
		...
		async addNoticeFile() {
			...
			axios
				.addNoticeFile(qs.stringify(this.noticeFile))
				.then(res => {
					console.log(res);
